...
Modified By Baoqiang Lao (lbq@shao.ac.cn)
"""
import os, warnings
try:
...
def download_G4Jy_samples():
    """
    Three examples to cutout Fornax A

    """
    csvfile = 'G4Jy_catalogue_18012020.csv' 
    dl_dir = './G4jy_hetu_img_double'

    with open('{}'.format(csvfile),'r') as f:
        reader = csv.reader(f)
        label = [row[10]for row in reader]

    with open('{}'.format(csvfile),'r') as f:
        reader = csv.reader(f)
        ra = [row[39]for row in reader]

    with open('{}'.format(csvfile),'r') as f:
        reader = csv.reader(f)
        dec = [row[41]for row in reader]
        
    label = np.array(label[1:len(label)+1])
    ra = np.array(ra[1:len(ra)+1])
    dec = np.array(dec[1:len(dec)+1])
